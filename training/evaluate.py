...
def parse_args():
    ...
    parser.add_argument(
        "--eval_root",
        type=str,
        help="Path to experiment directory containing a checkpoint to "
    ...
    parser.add_argument(
        "--load_dir", type=str, default="./models", help="path to pretrained ViT checkpoints."
    )
...
def score_nights_dataset(model, test_loader, device):
    ...
    targets = []
    with torch.no_grad():
        for i, (img_ref, img_left, img_right, target, idx) in tqdm(
            enumerate(test_loader), total=len(test_loader)
        ):
            img_ref, img_left, img_right, target = (
                img_ref.to(device),
                img_left.to(device),
                img_right.to(device),
                target.to(device),
...
def get_baseline_model(
    ...
    if baseline_model == "psnr":
        ...
        "clip" in baseline_model
        or "dino" in baseline_model
        or "open_clip" in baseline_model
        or "mae" in baseline_model
    ):
...
def run(args, device):
    ...
    if args.checkpoint_epoch is not None:
        ...
        logging.basicConfig(
            filename=os.path.join(args.baseline_output_path, "eval.log"), level=logging.INFO, force=True
        )
        model = get_baseline_model(
            args.baseline_model, args.baseline_feat_type, args.baseline_stride, args.load_dir, device
        )
    ...
    test_dataset_imagenet = TwoAFCDataset(
        root_dir=args.nights_root, split="test_imagenet", preprocess=get_preprocess(model_type)
    )
    test_dataset_no_imagenet = TwoAFCDataset(
        root_dir=args.nights_root, split="test_no_imagenet", preprocess=get_preprocess(model_type)
    )
    total_length = len(test_dataset_no_imagenet) + len(test_dataset_imagenet)
    test_imagenet_loader = DataLoader(
        test_dataset_imagenet, batch_size=args.batch_size, num_workers=args.num_workers, shuffle=False
    )
    test_no_imagenet_loader = DataLoader(
        test_dataset_no_imagenet, batch_size=args.batch_size, num_workers=args.num_workers, shuffle=False
    )
    ...
    eval_results["nights_total"] = (
        imagenet_score.item() * len(test_dataset_imagenet)
        + no_imagenet_score.item() * len(test_dataset_no_imagenet)
    ) / total_length
