...
from util.train_utils import HingeLoss, Mean, seed_worker
...
def parse_args():
    ...
    parser.add_argument(
        "--log_dir", type=str, default="./logs", help="path to save model checkpoints and logs"
    )
    ...
    ## Dataset options
    parser.add_argument(
        "--dataset_root", type=str, default="./dataset/nights", help="path to training dataset."
    )
...
class LightningPerceptualModel(pl.LightningModule):
    ...
    def on_train_epoch_end(self):
        ...
        self.logger.experiment.add_scalar(
            "train_loss/", self.epoch_loss_train / self.trainer.num_training_batches, epoch
        )
        self.logger.experiment.add_scalar(
            "train_2afc_acc/", self.train_num_correct / self.train_data_len, epoch
        )
        if self.use_lora:
    ...
    def on_validation_epoch_end(self):
        ...
        self.log("val_acc_ckpt", score, logger=False)
        self.log("val_loss_ckpt", loss, logger=False)
        # log for tensorboard
        self.logger.experiment.add_scalar("val_2afc_acc/", score, epoch)
        self.logger.experiment.add_scalar("val_loss/", loss, epoch)
    ...
    def configure_optimizers(self):
        ...
        for extractor, feat_type in zip(
            self.perceptual_model.extractor_list, self.perceptual_model.feat_type_list
        ):
    ...
    def load_lora_weights(self, checkpoint_root, epoch_load):
        for extractor in self.perceptual_model.extractor_list:
            load_dir = os.path.join(checkpoint_root, f"epoch_{epoch_load}_{extractor.model_type}")
    ...
    def __prep_lora_model(self):
        for extractor in self.perceptual_model.extractor_list:
            ...
            extractor_model = get_peft_model(ViTModel(extractor.model, ViTConfig()), config).to(
                extractor.device
            )
    ...
    def __save_lora_weights(self):
        for extractor in self.perceptual_model.extractor_list:
            save_dir = os.path.join(
                self.trainer.callbacks[-1].dirpath,
...
def run(args, device):
    ...
    if args.use_lora:
        exp_dir += (
            f"_lorar_{str(args.lora_r)}_loraalpha_{str(args.lora_alpha)}_loradropout_{str(args.lora_dropout)}"
        )
    ...
    train_dataset = TwoAFCDataset(
        root_dir=args.dataset_root, split="train", preprocess=get_preprocess(args.model_type)
    )
    val_dataset = TwoAFCDataset(
        root_dir=args.dataset_root, split="val", preprocess=get_preprocess(args.model_type)
    )
    ...
    trainer = Trainer(
        ...
        callbacks=ModelCheckpoint(
            monitor="val_loss_ckpt", save_top_k=-1, save_last=True, filename="{epoch:02d}", mode="max"
        ),
